...
def _get_subgames_states(state, all_states, depth_limit, depth,
  ...
  #if LOUD and int(state.current_player()) >= 0:
  #  print([state.information_state_string(state.current_player())])
  if state.is_terminal():
    LOUD and print("TERM:", state.history())
...
def _gas(game,
  ...
  if k not in _gasmap:
    ...
    _gasmap[k] = all_states
  else:
    print("  _gas hit")
  return _gasmap[k]

def get_all_states(*args, **kwargs):
  return _gas(*args, **kwargs)

def _get_all_states(game,
                   depth_limit=-1,
                   include_terminals=True,
                   include_chance_states=False,
                   include_mean_field_states=False,
